...
measure_direction() {
    ...
    for i in $(# gets a list of PIDs for child processes named 'netperf'
        pgrep -P $$ netperf
    ); do
        #echo "Waiting for $i"
        wait $i
...
while [ $# -gt 0 ]; do
    ...
    --)
        ...
        ;;
    *)
        echo "Usage: sh betterspeedtest.sh [-4 -6] [ -H netperf-server ] [ -t duration ] [ -p host-to-ping ] [ -n simultaneous-sessions ]"
        exit 1
        ;;
    esac
done
